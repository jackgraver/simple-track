...
import (
	...
	money "be-simpletracker/internal/core/money"
...
func CreateFeatures(db *gorm.DB, router *gin.Engine) {
	...
	moneyHandler := money.NewHandler(db)
	if err := moneyHandler.Migrate(); err != nil {
		panic(err)
	}
	moneyHandler.RegisterRoutes(router, authMW)
}
