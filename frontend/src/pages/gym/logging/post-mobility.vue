...
const { loggedPostMobility, pending, error, savePostMobility } = useWorkoutStore(offset);
...
const handleSave = async (checked: string[]) => {
    try {
        await savePostMobility(checked);
    } catch (err: any) {
...
<template>
    ...
    <div v-else-if="!loggedPostMobility" class="flex w-full flex-col gap-4 self-stretch">
        <div>No post-workout mobility for this day.</div>
        <button
    ...
    <div v-else class="container flex w-full flex-col gap-4 self-stretch">
        <MobilityLoggingView :logged-mobility="loggedPostMobility" @back="goBack" @save="handleSave" />
    </div>
</template>
